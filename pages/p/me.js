					...
					>
						...
						<div className="tabs tabs-boxed">
							<p
								...
								className={`tab rounded-full ${activeTab === "account" && "tab-active"}`}
							>
								Account Details
							</p>
							<p
								onClick={() => setActiveTab("settings")}
								className={`tab rounded-full ${activeTab === "settings" && "tab-active"}`}
							>
								Settings
							</p>
						</div>
								...
								>
									...
									<div className="grid grid-cols-1 lg:grid-cols-2 gap-2">
										...
										<div className="bg-base-200 p-5 rounded-btn max-h-max">
											<p className="text-lg font-bold text-primary">Company Details</p>
										...
										<div className="bg-base-200 p-5 rounded-btn max-h-max">
											...
											<p className="flex items-center justify-between">
												...
												<span>{session.user.user_metadata.contactInformation.email}</span>
											</p>

											<p className="flex items-center justify-between">
												<span>Company Phone</span>
												<span>{session.user.user_metadata.contactInformation.phone}</span>
											</p>
									...
									<div className="grid grid-cols-1 lg:grid-cols-2 gap-2">
										...
										<div>
											<div className="bg-base-200 p-5 rounded-btn">
												...
												<label htmlFor="signOutModal" className="btn btn-error btn-block">
													Sign out
												</label>
											</div>
										</div>
					...
					<label htmlFor="signOutModal" className="modal">
						<div className="modal-box relative">
							<div className="flex items-center justify-between">
								<h3 className="text-lg font-bold">Do you want to sign out from your account?</h3>
								<label htmlFor="signOutModal" className="btn fab fab-sm">
									<FiX />
								</label>
							</div>
							<p className="py-4">
								Signing out will end your session and you will have to sign in again to access your account.
							</p>
							...
							<div className="grid grid-cols-1 md:grid-cols-2 gap-2 mt-5">
								...
								<label onClick={signOutUser} htmlFor="signOutModal" className="btn btn-error btn-block">
									Sign out
								</label>
							</div>
						</div>
