...
const ProfilePage = () => {
	...
	const fetchUserDetails = async () => {
		const { data, error } = await __supabase.from("user_hunters").select("*").eq("id", session.user?.id).single();
							...
							<div className="col-span-3 flex flex-col gap-3">
								...
								<div className="flex items-center gap-2 flex-col sm:flex-row bg-base-200 rounded-btn p-5">
									...
									<div>
										<p className="text-3xl font-bold">
											{userDetails.data.full_name.first} {userDetails.data.full_name.last}
										</p>

										<p className="font-semibold opacity-75">@{userDetails.data.username}</p>
										<p>
											Joined at:{" "}
											<span className="opacity-50">
												{session && dayjs(session.user?.created_at).format("MMMM DD, YYYY")}
											</span>
										</p>
									</div>
								</div>
								...
								<div className="flex flex-col border-2 border-base-content border-opacity-30 rounded-btn p-5 gap-3">
									<div className="flex justify-between items-start">
										...
										<button onClick={() => setIsBioEditing(!isBioEditing)} className="btn btn-ghost btn-sm">
											<FiEdit />
										</button>
									</div>
									{!isBioEditing ? (
										<ReactMarkdown className="prose">
											{userDetails.data.bio || "This user has not added a bio yet"}
										</ReactMarkdown>
									) : (
										<form
											onSubmit={async (e) => {
								...
								<div className="flex flex-col border-2 border-base-content border-opacity-30 rounded-btn p-5 gap-3">
									...
									<div className="flex flex-col">
										...
										<p className="badge badge-primary">{userDetails.data.skill_primary}</p>
									</div>
									<div className="flex flex-col">
										<h4 className="text-lg font-semibold">Secondary Skills</h4>
										<p className="flex flex-wrap gap-4">
											{userDetails.data.skill_secondary.map((skill, index) => (
												<span key={`secondaryskill_${index}`} className="badge badge-accent">
													{skill}
												</span>
											))}
										</p>
								...
								<div className="flex flex-col border-2 border-base-content border-opacity-30 rounded-btn p-5 gap-3">
									...
									<div className="flex flex-col ">
										...
										<h4 className="text-lg font-semibold mt-3">City of Residence</h4>
										<p className="badge badge-accent">{userDetails.data.address.city}</p>
									</div>
								</div>
								{/* activity */}
								<div className="flex flex-col border-2 border-base-content border-opacity-30 rounded-btn p-5 gap-3">
									...
									<div className="flex flex-col gap-2">
										{userActivities.isSuccess &&
											userActivities.data.map((activity, index) => (
												...
												>
													<div>
														<ReactMarkdown>{activity.content.substring(0, 50)}</ReactMarkdown>
													</div>
													<Link href={`/h/feed/${activity.id}`} className="btn btn-primary btn-sm">
														See more
													</Link>
												</div>
											))}
									...
									{userConnections.isSuccess && (
										<div className="flex flex-col gap-2">
											{userConnections.data.length < 1 && (
												<p>
													Looks like you have not connected to other people right now. Add people to your connections to
													see their posts and activities.
												</p>
											)}
										</div>
									)}
												...
												<div className="flex gap-2 items-center">
													...
													<div>
														<p className="font-bold leading-none">
															{connection.full_name.first} {connection.full_name.last}
														</p>
														<p className="opacity-50 leading-none">@{connection.username}</p>
													</div>
												</div>
											</Link>
										))}
                  ...
                  userConnections.data.length > 0 ? (
                      ...
                      >
                        See all your connections
								...
								<div className="flex flex-col rounded-btn p-2 gap-3">
									...
									<div className="flex flex-col gap-2">
										{recommendedUsers.isSuccess && recommendedUsers.data.length < 1 && (
											<p>
												We do not have any suggestions for you right now. Try connecting to more people to get more
												suggestions.
											</p>
										)}
										...
										{recommendedUsers.isSuccess &&
											...
											recommendedUsers.data.map((thisUser, index) => (
												<Link href={`/h/${thisUser.username}`} key={`connection_${index}`}>
													<div className="flex gap-5 items-center justify-between p-3 bg-base-200 rounded-btn hover:bg-base-300">
														<div className="flex gap-5 items-center">
															...
															<div>
																...
																<p className="opacity-50 leading-none">@{thisUser.username}</p>
															</div>
														</div>
													</div>
												</Link>
								...
								<div className="flex flex-col rounded-btn p-2 gap-5">
									<label className="flex items-center justify-between">
										<span>Dark Mode</span>
										<input type="checkbox" className="toggle" checked={isDark} onChange={toggleTheme} />
									</label>
					...
					<div className="modal">
						<div className="modal-box">
							...
							<p className="py-4">You will be signed out of all your devices. You can sign back in anytime.</p>
							<div className="modal-action">
								<label htmlFor="signoutmodal" className="btn">
									Cancel
								</label>
								<button className="btn btn-error" onClick={handleLogout} disabled={isLoggingOut}>
									Sign out
								</button>
							</div>
						</div>
