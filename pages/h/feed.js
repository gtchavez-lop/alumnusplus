...
const FeedPage = () => {
	...
	const currentUser = useQuery({
		...
		queryFn: async () => {
			const { data, error } = await __supabase.from("user_hunters").select("*").eq("id", thisSession.user.id).single();
			if (error) {
				return null;
			}
			return data;
	...
	const fetchFeed = async () => {
		...
		const { data, error } = await __supabase
			.from("public_posts")
			.select("id,content,comments,createdAt,updatedAt,uploader(id,email,full_name,username),upvoters")
					...
					>
						...
						<div className="col-span-full lg:col-span-3 ">
							...
							<div className="flex gap-2 w-full items-center">
								...
								/>
								<div onClick={() => setIsMakingPost(true)} className="btn btn-primary btn-block max-w-md">
									Create Post
								</div>
							</div>
							...
							<div className="mt-10">
								<div className="flex flex-col gap-5">
									...
									{feedList.isSuccess && feedList.data.map((item) => <FeedCard data={item} key={item.id} />)}
								</div>
							</div>
						</div>
						...
						<div className="col-span-full lg:col-span-2">
							<div className="flex flex-col rounded-btn py-2 gap-3">
								...
								<div className="flex flex-col gap-2">
									{recommendedUsers.isSuccess && recommendedUsers.data.length < 1 ? (
										<p>
											Looks like you have not connected to other people right now. Add people to your connections to see
											...
											{recommendedUsers.isSuccess &&
												recommendedUsers.data.map((thisUser, index) => (
													...
													>
														...
														<Link href={`/h/${thisUser.username}`} className="btn btn-sm btn-primary">
															See Profile
														</Link>
													</div>
												))}
							...
							<div className="grid grid-cols-2 mt-5 font-light text-sm opacity-50">
								<div className="flex flex-col">
									...
									<Link href="/util/features#companyhunting" className="link link-hover">
										Geo-Company Hunting
									</Link>
						...
						>
							...
							>
								<div className="flex justify-between items-center">
									<motion.p className="text-primary text-lg font-bold">Create a blog post</motion.p>
									<motion.button onClick={(e) => setIsMakingPost(false)} className="btn btn-error">
										<FiX />
									</motion.button>
								</div>
								...
								>
									...
									<div className="flex justify-end gap-2">
										<button type="button" onClick={(e) => setIsMakingPost(false)} className="btn btn-error">
											Cancel
										</button>
										<motion.button
